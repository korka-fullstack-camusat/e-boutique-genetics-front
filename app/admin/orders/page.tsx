...
import { ShoppingCart, Eye, X, CheckCircle, XCircle, Search, ChevronLeft, ChevronRight, FileText, Send, Loader2 } from "lucide-react";
...
export default function AdminOrdersPage() {
  ...
  function openInvoice(order: Order) {
    setAdminInvoice({
      ...
      acompteAmount:  order.acompte_amount ?? null,
    });
    setAdminInvoiceOpen(true);
    setGeneratedInvoices((prev) => new Set([...prev, order.id]));
  }
  ...
  return (
    <>
              ...
              <div className="p-5 space-y-5">
                ...
                {isPending ? (
                  ...
                  <div className="space-y-3">
                    <div className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold ${STATUS_STYLES[selected.status] ?? "bg-gray-100 text-gray-600"}`}>
                      {STATUS_LABELS[selected.status] ?? selected.status}
                    </div>

                    {isConfirmed && (
                      <div className={`grid gap-3 ${generatedInvoices.has(selected.id) ? "grid-cols-2" : "grid-cols-1"}`}>
                        <button
                          onClick={() => openInvoice(selected)}
                          className="flex items-center justify-center gap-2 py-3 bg-amber-500 text-gray-900 rounded-xl font-bold text-sm hover:bg-amber-400 transition-colors"
                        >
                          <FileText size={16} /> Générer facture
                        </button>
                        {generatedInvoices.has(selected.id) && (
                          <button
                            onClick={() => handleSendInvoice(selected)}
                            disabled={sendingInvoice}
                            className="flex items-center justify-center gap-2 py-3 bg-blue-600 text-white rounded-xl font-bold text-sm hover:bg-blue-700 transition-colors disabled:opacity-60"
                          >
                            {sendingInvoice
      ...
      {/* Facture admin (z-[60] — au-dessus du modal commande) */}
      <InvoiceModal
        open={adminInvoiceOpen}
        data={adminInvoice}
        onClose={() => setAdminInvoiceOpen(false)}
      />
    </>
  );
}
